...
if ( ! function_exists('lib_url'))
{
    function lib_url()
    {
        ...
        $CI =& get_instance();
        //return the full asset path
        return base_url() . $CI->config->item('lib_path');
    }
}
/**
 * Get css URL
...
if ( ! function_exists('lib_path'))
{
    function lib_path()
    {
        //get an instance of CI so we can access our configuration
        $CI =& get_instance();
        return FCPATH . $CI->config->item('lib_path');
    }
}
...
/**
 * Load lib
 * Creates the <link> tag that links all requested lib file
 * @access  public
 * @param   string
 * @return  string
 */
if ( ! function_exists('lib_css'))
{
    function lib_css($file, $media='all')
    {
        return '<link rel="stylesheet" type="text/css" href="' . lib_url() . $file . '" media="' . $media . '">'."\n";
    }
}
 ...
 * @return  string
 */
if ( ! function_exists('lib_js'))
{
    function lib_js($file, $atts = array())
    {
        $element = '<script type="text/javascript" src="' . lib_url() . $file . '"';
 ...
 * @return  string
 */
if ( ! function_exists('lib_img'))
{
    function lib_img($file,  $atts = array())
    {
		$url = '<img src="' . lib_url() . $file . '"';
		foreach ( $atts as $key => $val )
			$url .= ' ' . $key . '="' . $val . '"';
		$url .= " />\n";
        return $url;
